...
export default function AjudaSuporteScreen() {
  ...
  const loadLiveChecks = async () => {
    ...
    try {
      const gpsStatus = await getAppGpsStatus();

      const gpsLabel =
        gpsStatus === 'enabled'
          ? supportText('gpsReady', language)
          : gpsStatus === 'disabled'
            ? supportText('gpsServiceDisabled', language)
            : gpsStatus === 'denied'
              ? supportText('gpsPermissionDenied', language)
              : supportText('notChecked', language);

      setDiagnostic((prev) => ({ ...prev, gps: gpsLabel }));
    } catch {
  ...
  const statusColor = (value: string) => {
    ...
    if (normalized.includes('permissão negada') || normalized.includes('permission denied') || normalized.includes('permiso denegado')) {
      return '#EF4444';
    }

    if (normalized.includes('offline') || normalized.includes('sin conexión') || normalized.includes('inativo') || normalized.includes('inactive') || normalized.includes('pendente') || normalized.includes('pending') || normalized.includes('indisponível') || normalized.includes('unavailable') || normalized.includes('no disponible') || normalized.includes('gps desligado') || normalized.includes('gps disabled') || normalized.includes('gps apagado')) {
      return '#F59E0B';
    }
