...
4. Usuário acessa o login ou, se já houver sessão válida, a área autenticada.
5. Sincronização baixa roteiro, pesquisas, justificativas, alertas e dados complementares.
6. Usuário executa visita, pesquisa e evidências mesmo offline, quando os dados necessários já estão sincronizados localmente.
7. Ações pendentes são enfileiradas em SQLite.
8. Quando há rede, a fila é enviada para a API e o roteiro local é atualizado com o retorno remoto.
...
A regra final de permissão de acesso mobile validada pelo backend: A confirmar.

## Roteiro de visitas

Fluxo identificado:

1. `globalSync` busca o roteiro em endpoints relacionados ao projeto/promotor.
...
7. Atualiza a UI local imediatamente, antes da confirmação definitiva do backend.
...
1. Usuário inicia check-out em visita já iniciada/em andamento.
...
Validação de obrigatoriedade de pesquisa antes do check-out: A confirmar.
...
5. Ao concluir, o app monta o payload de coleta e enfileira o envio para `/coletas` via `sync_queue`.
6. Coletas locais ficam na tabela `coletas` com status, respostas, erro de sync e contadores de tentativa.

Tipos exatos de pergunta suportados: A confirmar. O código normaliza tipos como texto, número, opções e produtos.
...
- Fotos locais são mantidas por URI até a sincronização/upload.
- Antes do envio, o sync troca URIs locais por URLs de armazenamento obtidas via upload.
...
Tudo que precisa ser enviado ao backend é mantido em `sync_queue` ou marcado como `pending_sync`, dependendo do fluxo.
...
2. Coleta telemetria.
3. Processa a fila local em ordem de criação.
4. Faz upload de fotos locais antes do envio de payloads.
5. Envia payloads para os endpoints gravados na fila.
6. Remove itens enviados com sucesso ou reconhecidos como já processados.
...
- Em conflitos, servidor e local são conciliados por status/timestamps; quando datas estão ausentes, a decisão pode depender de fallback e merece atenção em testes.
## Pontos a confirmar

- Regra final de permissão mobile validada pelo backend.
- Validação definitiva de obrigatoriedade de pesquisas antes do check-out.
- Política de retenção/limpeza de fotos locais após upload.
- Endpoints exatos de confirmação de leitura/aceite de alertas.
- Persistência local completa da tela de performance.
- Equivalência exata entre cálculo local e backend para Perfect Store.
