...
- **Modelo geral:** offline-first para roteiro, visitas, pesquisas, coletas/respostas, justificativas, alertas, campanhas/performance, Perfect Store, logs e fila de sincronização.
...
| `alerts` | Alertas locais, incluindo leitura e aceite offline. | `id`, `titulo`, `conteudo`, `remetente_nome`, `data_envio`, `prioridade`, `lida`, `lida_em`, `exige_aceite`, `aceita_em`, `raw_json`, `updated_at`. |
...
Alertas são armazenados na tabela `alerts`. A sincronização baixa alertas do backend e chama o salvamento offline, que normaliza os dados e faz persistência local preservando estados já existentes de leitura e aceite (`lida`, `lida_em`, `aceita_em`).
...
Não há, no trecho analisado, limite máximo explícito de tentativas nem política de expurgo automático por idade da fila. **A confirmar** se existe em outro ponto do app ou no backend.
...
O código tenta tolerar cenários de instalações antigas ou inicializações simultâneas: erros de coluna duplicada/coluna já existente são ignorados. Algumas falhas de migração são registradas em `console.warn` e tratadas para não interromper o fluxo principal. Mesmo assim, devem ser monitoradas, pois podem indicar divergência de schema local.

Não há versionamento formal de schema com tabela de migrations ou número de versão identificado no trecho analisado. O versionamento aparente está no nome do arquivo `app_coleta_v16.db`. **A confirmar** se há outro mecanismo fora de `src/database/db.ts`.
...
- **Logs locais:** `app_logs` tem retenção dos registros mais recentes para não inflar o SQLite; a regra exata de retenção deve ser confirmada no código atual.
