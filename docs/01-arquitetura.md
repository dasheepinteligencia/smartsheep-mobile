...
4. **Persistência local** em `src/database/db.ts`, usando Expo SQLite.
...
- `index`: tela inicial/dashboard. Detalhes completos: A confirmar.
- `roteiro`: lista do roteiro de visitas.
- `alertas`: mensagens/alertas.
...
- retorna o objeto `Response`, inclusive em erros HTTP, mantendo compatibilidade com consumidores que leem status/body.
...
Uso de HTTP em fallback: ponto de atenção de segurança.
...
Também existe sincronização em segundo plano registrada no layout raiz como `BACKGROUND_SYNC_TASK`, com `minimumInterval` de 15 minutos, `stopOnTerminate: false` e `startOnBoot: true`.
...
- envia o arquivo e retorna a URL final para substituir o campo no payload.
...
- Dependência de sincronização correta entre fila local, flags `pending_sync` e dados remotos.
...
- Background sync não é garantido por todos os dispositivos/sistemas.
- Uso de múltiplos formatos de payload e campos legados aumenta complexidade de manutenção.
