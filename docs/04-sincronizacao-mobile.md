...
A sincronização central é executada por `globalSync()` em `src/services/syncService.ts`. O fluxo é offline-first: ações feitas no aparelho são persistidas no SQLite, enfileiradas quando necessário e enviadas ao backend antes de atualizar o espelho local com dados remotos.
...
Se a rede não estiver disponível, `globalSync()` retorna sem processar a fila e sem baixar dados. As pendências permanecem no SQLite para tentativas futuras.
...
4. recebe/usa `fileUrl` como URL pública/armazenada;
...
- Requisições HTTP não-OK continuam retornando `Response` pelo serviço `api()`, permitindo que o sync leia `status` e corpo.
...
Quando isso acontece, `shouldRemoveFromQueue()` permite remover o item da fila. O objetivo aparente é tornar o envio idempotente para casos em que o backend já recebeu a operação anteriormente, mas o app ainda manteve a fila local.
...
- Se o vencedor do conflito é `LOCAL`, o app preserva o estado operacional local e apenas mescla campos informativos do servidor.
...
- **Fila sem limite máximo aparente:** `attempts` aumenta, mas não foi observado limite global para parar tentativas.
...
- **Tratamento específico de estoque:** erros de saldo insuficiente em coletas removem a fila e liberam correção, mas outros erros podem ficar tentando indefinidamente.
...
- Como suporte/operacional deve tratar itens com `attempts` alto e `last_error` recorrente.
