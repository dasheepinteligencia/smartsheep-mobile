...
O helper `addToSyncQueue()` adiciona metadados quando usado, incluindo `created_offline_at`, `origem` padrão e `client_operation_id` caso falte. A pesquisa avulsa não usa esse helper no trecho analisado; ela insere o payload diretamente na fila.
...
- fotos de perguntas parecem ser incorporadas como base64 no payload de coleta, sem depender de upload separado de mídia para esses anexos no trecho analisado.
...
- Se fotos de perguntas devem sempre ser enviadas como base64 em `/coletas` ou se haverá upload dedicado de evidências de pesquisa no futuro.
