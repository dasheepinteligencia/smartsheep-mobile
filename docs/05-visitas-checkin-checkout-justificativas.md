...
4. Obtém a localização do usuário para exibir no mapa e validar o check-in.
...
A confirmar: se existe também uma validação equivalente no backend.
...
- durante a sincronização, se o valor for `file://`, a foto é enviada para AWS e o payload é atualizado com a URL pública.
...
- a sincronização faz upload para AWS antes de enviar ao backend, quando o campo ainda é `file://`.
...
Fotos obrigatórias também são persistidas localmente antes da sincronização. Se a foto estiver como arquivo local (`file://`), a sincronização tenta fazer upload para AWS e substituir o campo por URL pública antes de enviar o payload ao backend.
...
- se fotos locais foram enviadas para AWS, atualiza o campo de foto na tabela `visits` com a URL pública;
...
- **Justificativa local incompleta na tabela**: o payload da fila contém motivo/detalhe, mas o `UPDATE visits` da ação de justificativa não grava explicitamente esses campos na tabela naquele momento. A confirmar se isso é intencional.
- **Fallback de justificativas fixas**: se a sincronização ainda não baixou motivos, o app usa motivos hardcoded, que podem não existir no backend.
...
- **Bloqueio de múltiplas visitas é local**: se houver outro dispositivo/sessão ou ação web, a confirmação final depende do backend. A confirmar validação no servidor.
